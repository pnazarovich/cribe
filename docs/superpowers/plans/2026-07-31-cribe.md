# Cribe Implementation Plan

> Документ написан до переименования продукта (Transcriber → Cribe, август 2026): имена целей,
> путей и идентификаторов приведены к текущим, всё остальное — как было.
...
**Architecture:** SPM-пакет без .xcodeproj: библиотека `CribeCore` (вся логика, тестируется `swift test`), executable `Cribe` (SwiftUI MenuBarExtra + NSPanel), executable `cribe-cli` (конвейер на WAV-файлах). Сборка .app — `xcodebuild` прямо по Package.swift + ручной бандл + codesign (рецепт проверен на этой машине).
...
- Спека: `docs/superpowers/specs/2026-07-31-cribe-design.md` — истина при конфликте.
- Bundle id: `online.nazarovych.cribe` (конвенция пользователя). CFBundleName/продукт: `Cribe`.
- Подпись ТОЛЬКО: `<ваш сертификат подписи>` (единственная валидная identity; ad-hoc запрещён — сбрасывает TCC при каждой сборке).
- Сборка app: `xcodebuild -scheme Cribe` (НЕ `swift build` — resource-bundle KeyboardShortcuts крашит SPM-аксессор; бандл копировать в `Contents/Resources/`). `swift build`/`swift test` — только для core/cli/тестов.
...
Cribe/
  ...
  scripts/build-app.sh            # xcodebuild → dist/Cribe.app + codesign
  Sources/
    CribeCore/
...
**Files:** Create: `Package.swift`, `Info.plist`, `scripts/build-app.sh`, `Sources/CribeCore/Language.swift`, `Sources/Cribe/App.swift` (минимальный MenuBarExtra-стаб), `Sources/CribeCLI/CLI.swift` (стаб print), `Tests/CribeCoreTests/ReplacementEngineTests.swift` (1 плейсхолдер-тест `XCTAssertTrue(true)` — заменится в Task 2).
...
- [ ] **Step 1: Package.swift** — tools 5.10, platforms `[.macOS(.v14)]`; targets: `CribeCore` (deps: WhisperKit product из argmax-oss-swift, FluidAudio), `Cribe` executable (deps: CribeCore, KeyboardShortcuts), `CribeCLI` executable (deps: CribeCore), тест-таргет. Пины из Global Constraints.
- [ ] **Step 2: Info.plist** — точный plist из research (CFBundleIdentifier `online.nazarovych.cribe`, LSUIElement true, NSMicrophoneUsageDescription на русском, LSMinimumSystemVersion 14.0, CFBundleShortVersionString 0.1.0).
...
echo "OK: $APP"
```
- [ ] **Step 4:** `.gitignore`: `.build/`, `.ddata/`, `dist/`. App.swift: `@main struct CribeApp: App { var body: some Scene { MenuBarExtra("Cribe", systemImage: "mic") { Text("stub") } } }`.
- [ ] **Step 5:** `swift build && swift test` → зелёные; `bash scripts/build-app.sh` → `OK: dist/Cribe.app`. Коммит `feat(cribe): скаффолд SPM-проекта, сборка и подпись .app`.
...
public final class UserDictionary: @unchecked Sendable {          // сериализация через DispatchQueue
  public static let defaultURL: URL  // ~/Library/Application Support/Cribe/dictionary.json
...
- [ ] Step 1: тесты (falling); Step 2: `swift test` FAIL; Step 3: реализация; Step 4: `swift test` PASS; Step 5: коммит `feat(cribe): словарь — замены со склонениями и prompt-биасинг`.
...
- [ ] Реализация → `swift build` зелёный (юнит-тестов нет — обёртки над HW/CoreML) → коммит `feat(cribe): аудиозахват 16кГц и Silero VAD (автостоп, обрезка тишины)`.
...
- [ ] TDD-цикл → `swift test` PASS → коммит `feat(cribe): GPT-слой — Codex OAuth device flow, API-key, SSE, модели, effort`.
...
- [ ] Реализация → build → коммит `feat(cribe): вставка через буфер+Cmd-V, secure input detect`.
...
**Конвейер toggle():** idle → (модель не готова: prepare с прогрессом) → start запись: AudioRecorder.start, чанки → VadGate.feedStream (speechEnd → авто-toggle) и в буфер; live-превью: таймер-цикл каждые 1.2 c, если предыдущий проход завершён — `engine.transcribe(текущий буфер, prompt)` в фоне → `state = .recording(live: text)` (пропускать, если буфер < 1 c). Повторный toggle/автостоп → stop: финал = `VadGate.trimmed(буфер)`; nil → `.error("речь не обнаружена")` → idle через 2 c. Иначе `.transcribing` → финальный transcribe → ReplacementEngine.apply → если gptEnabled: `.cleaning` → PostProcessor.cleanup (catch → результат слоя 2 + `.degraded(причина)`) → TextInserter.insert → HistoryStore.add → `.inserted` → idle через 1.5 c. WAV последней записи писать в `~/Library/Application Support/Cribe/last-recording.wav` до успешной вставки (простая WAV-запись 16-bit PCM руками, ~30 строк). Прогресс/ошибки — только через `state` (UI подписан).

- [ ] Реализация → build → коммит `feat(cribe): конвейер диктовки — состояния, live-превью, деградации`.
...
- [ ] Реализация → build → коммит `feat(cribe): live-панель`.
...
- [ ] Реализация → build + `bash scripts/build-app.sh` → коммит `feat(cribe): UI — меню-бар, настройки с Codex-авторизацией, словарь, онбординг`.

---

### Task 10: CLI + фикстуры + smoke

**Files:** Modify: `Sources/CribeCLI/CLI.swift`.

`cribe-cli <file.wav|.m4a|.aiff> --lang ru|uk [--no-gpt] [--no-vad]`: загрузка аудио `AudioProcessor.loadAudioAsFloatArray(fromPath:)` (WhisperKit, сам ресемплит в 16k) → `DictationController.process(fileSamples:...)` (без вставки) → печать финального текста в stdout, стадии в stderr. Смоук локально: сгенерировать фикстуру `say -v Milena "Сделай коммит и задеплой на гитхаб через докер" -o fixture.aiff` (голос Milena есть в macOS; если нет — `say -v '?'` выбрать русский), прогнать `--lang ru --no-gpt`: выход обязан содержать `GitHub`/`deploy` латиницей после слоя 2. Это скачает turbo (~1.6 GB) — ок, это же прогрев приложения.

- [ ] Реализация + smoke-прогон (вывод в лог задачи) → коммит `feat(cribe): CLI-прогон конвейера + smoke на синтетической фразе`.
...
- [ ] TDD → реализация → `swift test` PASS → коммит `feat(cribe): перевод на английский через GPT-слой`.

---

### Task 13: Выбор микрофона (устройство ввода)

**Files:** Create: `Sources/CribeCore/Audio/AudioDeviceList.swift`; Modify: `Audio/AudioRecorder.swift`, `Support/AppSettings.swift`.
...
- [ ] Реализация → `swift build` (юнит-тестов нет — CoreAudio) → коммит `feat(cribe): выбор микрофона в меню`.
...
- [ ] Реализация → `swift build` + прослушать вручную нельзя (агент) — проверка: WAV-данные валидны (парсинг заголовка тестом), длительности в ожидаемых пределах → коммит `feat(cribe): звуки старта и окончания записи`.

---

### Task 16: Хоткей записи — правый ⌘ по умолчанию

**Files:** Create: `Sources/Cribe/ModifierKeyTap.swift`; Modify: `Support/AppSettings.swift` (+`dictationHotkeyMode: HotkeyMode` enum rightCommand|custom, default rightCommand, key `dictationHotkeyMode`), `Sources/Cribe/App.swift` (установка/снятие тапа по режиму), `Sources/Cribe/SettingsView.swift` (пикер режима; Recorder виден только в custom).
