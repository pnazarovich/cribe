...
swift run CribeCLI /tmp/fx.aiff --lang ru --no-gpt
```

## Building the app

**Use `bash scripts/build-app.sh`, never `swift build`, to produce `Cribe.app`.**
...
| Layer | Path | Rule |
| --- | --- | --- |
| Core logic | `Sources/CribeCore/` | No AppKit, no SwiftUI. Anything that can be a pure function is one. |
| ASR + audio | `Sources/CribeCore/{Audio,ASR}/` | Recording, VAD, WhisperKit engines. |
| Dictionary | `Sources/CribeCore/Dictionary/` | The three layers: `PromptBuilder`, `ReplacementEngine`, and the GPT prompt. |
| GPT | `Sources/CribeCore/GPT/` | API-key and ChatGPT-account auth, `SecretStore`, post-processing. |
| Pipeline | `Sources/CribeCore/Pipeline/` | `DictationController` — the state machine that wires it all together. |
| App | `Sources/Cribe/` | Menu bar, HUD panel, dictation cards, dictionary editor. |
| CLI | `Sources/CribeCLI/` | Headless pipeline runner. |

Tests mirror that split: `Tests/CribeCoreTests/` for the core, `Tests/CribeAppTests/`
