...
public final class AudioRecorder: AudioCapturing {
    ...
    private let logger = Logger(subsystem: "online.nazarovych.cribe", category: "AudioRecorder")
    ...
    /// Длина уже записанного. Отдельно от `capturedSamples` намеренно: снимок берёт вторую
    /// ссылку на массив, и ближайший `append` на аудиопотоке платит за копию по записи —
    /// malloc и memcpy всего буфера в реальном времени. Опрашивать надо этим, копировать —
    /// только когда проход действительно стартует.
    ...
    /// поэтому индексы сэмплов в промежуточном проходе и в финальном буфере совпадают.
    ...
    /// виден в `log show --predicate 'subsystem == "online.nazarovych.cribe"'`.
