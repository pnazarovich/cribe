...
public final class SecretStore: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "online.nazarovych.cribe.secrets")
...
public struct KeychainAccess: Sendable {
    ...
    /// Имя сервиса — идентификатор бандла: под ним секреты лежат в связке и под ним же их
    /// ищет система. Зашитая строка пережила бы переименование продукта и молча увела бы
    /// секреты в чужой сервис; запасное значение — для сборок без бандла (CLI, тесты).
    private static let service = Bundle.main.bundleIdentifier ?? "online.nazarovych.cribe"
