...
public enum DictionaryJudge {
    ...
    public static let systemPrompt = """
        ...
        с его секундой. Ты видишь всё: что услышало распознавание, что в итоге вставлено,
        поле сразу после вставки, ход изменений и поле в конце.

        Текстов два, и разница между ними — суть задачи. Распознавание слышит слово на
        слух, а потом другая модель причёсывает фразу и часто заменяет непонятное ей слово
        похожим. Словарь применяется РАНЬШЕ причёсывания — то есть к тому, что услышало
        распознавание. Значит, левой половиной пары обязано быть слово ИЗ УСЛЫШАННОГО
        текста, даже если человек правил уже причёсанное.

        Пример: услышано «клайв», вставлено «scribe», человек исправил на «Cribe».
        Правильная пара — «клайв» → «Cribe». Пара «scribe» → «Cribe» не сработает никогда:
        слова «scribe» в услышанном тексте не бывает.
        ...
        - человек заменил одно слово одним словом;
        - левое слово взято из услышанного текста — то самое, из которого выросла ошибка;
    ...
    public static func input(_ observation: FieldObservation, language: Language) -> String {
        var parts = [
            ...
            "Распознавание услышало:\n\(observation.recognized)",
            "После причёсывания в поле вставлено:\n\(observation.dictated)",
        ...
        if !observation.corrections.isEmpty {
            ...
            // молчит там, где правки идут подряд. Вдобавок он считает по ВСТАВЛЕННОМУ
            // тексту, то есть левая половина у него — слово причёсанное, а не услышанное.
            // Судья вправе предложить и то, чего в списке нет, и обязан перевести левую
            // половину на язык распознавания.
            parts.append(
                """
                Разбор нашёл однословные замены во вставленном тексте \
                (могут быть не все и не все верны, левая половина здесь — причёсанное слово):
                \(list)
                """
            )
        }
    ...
    /// услышали или вставили, а правильное — словом, которое человек и правда написал.
    ///
    /// Оба наших текста годятся источником: когда причёсывание слова не тронуло, они
    /// совпадают, а когда тронуло — верна версия распознавания, но настаивать на ней
    /// проверкой нельзя. Отсечь причёсанное слово значило бы отсечь и правильную пару
    /// в тот раз, когда причёсывания не было.
    public static func confirmed(_ proposals: [Proposal], in observation: FieldObservation) -> [Correction] {
        let ours = Set(
            (EditDiff.words(in: observation.recognized) + EditDiff.words(in: observation.dictated))
                .map { $0.lowercased() }
        )
