...
public enum Language: String, CaseIterable, Codable, Sendable {
    ...
    /// украинская вставка в русском окне выходит фонетическим мусором (см. `SecondOpinion`).
    /// Английскому пары нет — с ним такой беды не наблюдалось.
    ///
    /// Пара пока односторонняя, и это не забывчивость. Второе мнение спрашивают у модели
    /// соседа, а у русской сессии сосед — украинский large-v3, то есть модель СИЛЬНЕЕ
    /// собственной turbo. В обратную сторону вышло бы наоборот: украинскую речь судила бы
    /// turbo, которая, наоборот, тянет украинское в русское написание. Замеров для этого
    /// направления нет, а без них включать нельзя — на этой задаче уже дважды «чинили»
    /// по недомеру (см. коммит 441156a).
    public var neighbour: Language? {
        switch self {
        case .ru: return .uk
        case .uk, .en: return nil
        }
    }
}
